...
<div class="max-w-4xl space-y-6">
    ...
    <div class="glass-card rounded-3xl p-6 sm:p-8 space-y-6">
        <div>
            <h1 class="text-xl font-display font-bold text-charcoal-950">Edit Halaman Statis</h1>
            <p class="text-xs text-charcoal-500 font-light">Perbarui konten dan metadata halaman informasi.</p>
        </div>

        <form action="{{ route('admin.cms.pages.update', $page) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="content" rows="10" required
                    class="w-full bg-white/90 border border-cream-300 rounded-2xl p-3.5 text-xs text-charcoal-950 focus:outline-none focus:border-cream-500 leading-relaxed">{{ old('content', $page->content) }}</textarea>
            </div>
            ...
            <div class="flex items-center pt-2">
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $page->is_active) ? 'checked' : '' }} class="rounded border-cream-300 text-charcoal-950 shadow-sm focus:ring-cream-500">
                    <span class="ml-2 text-xs font-bold text-charcoal-800">Halaman Aktif</span>
                </label>
            </div>
