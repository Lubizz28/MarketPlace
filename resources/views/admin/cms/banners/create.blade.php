...
@section('title', 'Tambah Banner Promosi Baru')
...
<div class="max-w-3xl space-y-6">
    ...
    <div class="glass-card rounded-3xl p-6 sm:p-8 space-y-6">
        <div>
            <h1 class="text-xl font-display font-bold text-charcoal-950">Tambah Hero Banner Baru</h1>
            <p class="text-xs text-charcoal-500 font-light">Banner visual akan ditampilkan pada carousel halaman utama storefront.</p>
        </div>

        <form action="{{ route('admin.cms.banners.store') }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" required placeholder="Contoh: Koleksi Syar'i Ramadhan 1448 H"
            ...
            <div>
                ...
                <input type="text" name="subtitle" value="{{ old('subtitle') }}" placeholder="Contoh: Diskon hingga 30% untuk Member & Reseller"
                    class="w-full bg-white/90 border border-cream-300 rounded-2xl py-2.5 px-3.5 text-xs text-charcoal-950 focus:outline-none focus:border-cream-500">
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="button_text" value="{{ old('button_text', 'Belanja Sekarang') }}" placeholder="Contoh: Belanja Sekarang"
                        class="w-full bg-white/90 border border-cream-300 rounded-2xl py-2.5 px-3.5 text-xs text-charcoal-950 focus:outline-none focus:border-cream-500">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="button_url" value="{{ old('button_url', '/catalog') }}" placeholder="Contoh: /catalog atau /catalog?category=abaya"
                        class="w-full bg-white/90 border border-cream-300 rounded-2xl py-2.5 px-3.5 text-xs text-charcoal-950 font-mono focus:outline-none focus:border-cream-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                ...
                <div class="flex items-center pt-6">
                    <label class="inline-flex items-center cursor-pointer">
                        ...
                        <span class="ml-2 text-xs font-bold text-charcoal-800">Publikasikan Banner Langsung</span>
                    </label>
                </div>
            </div>
