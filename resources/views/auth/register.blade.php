...
<div x-data="{ role: '{{ $isReseller ? 'reseller' : 'member' }}' }">
    <!-- Role Switcher Tabs with Custom Vector Icons -->
    ...
    <div x-show="role === 'reseller'" x-cloak class="mb-5 p-4 bg-gold-50/90 border border-gold-400/50 rounded-2xl text-xs text-gold-950 space-y-1.5 shadow-xs">
        ...
        <p class="text-[11px] leading-relaxed text-sand-700">&bull; Potongan harga grosir langsung &amp; komisi referral otomatis.</p>
        <p class="text-[11px] leading-relaxed text-sand-700">&bull; Dompet saldo digital dengan penarikan dana transparan.</p>
    </div>
    ...
    <form method="POST" action="{{ route('register.post') }}" class="space-y-4">
        ...
        <div>
            <label for="name" class="block text-[10px] font-bold uppercase tracking-[0.2em] text-sand-700 mb-1.5">Nama Lengkap</label>
            <div class="relative">
                <input type="text" id="name" name="name" value="{{ old('name') }}" required
                    placeholder="cth. Siti Fatimah"
                    class="w-full bg-sand-50/80 border @error('name') border-rose-400 bg-rose-50/30 @else border-sand-200/90 @enderror rounded-2xl py-3 pl-11 pr-4 text-xs tracking-wide focus:outline-none focus:ring-2 focus:ring-emerald-900 focus:bg-white transition-all shadow-xs">
        ...
        <div>
            <label for="email" class="block text-[10px] font-bold uppercase tracking-[0.2em] text-sand-700 mb-1.5">Alamat Email</label>
            <div class="relative">
                <input type="email" id="email" name="email" value="{{ old('email') }}" required
                    placeholder="nama@email.com"
                    class="w-full bg-sand-50/80 border @error('email') border-rose-400 bg-rose-50/30 @else border-sand-200/90 @enderror rounded-2xl py-3 pl-11 pr-4 text-xs tracking-wide focus:outline-none focus:ring-2 focus:ring-emerald-900 focus:bg-white transition-all shadow-xs">
                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-sand-400">
        ...
        <div>
            <label for="phone" class="block text-[10px] font-bold uppercase tracking-[0.2em] text-sand-700 mb-1.5">Nomor WhatsApp / HP</label>
            <div class="relative">
                <input type="tel" id="phone" name="phone" value="{{ old('phone') }}" required
                    placeholder="081234567890"
                    class="w-full bg-sand-50/80 border @error('phone') border-rose-400 bg-rose-50/30 @else border-sand-200/90 @enderror rounded-2xl py-3 pl-11 pr-4 text-xs tracking-wide focus:outline-none focus:ring-2 focus:ring-emerald-900 focus:bg-white transition-all shadow-xs">
                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-sand-400">
        ...
        <div>
            <label for="password" class="block text-[10px] font-bold uppercase tracking-[0.2em] text-sand-700 mb-1.5">Kata Sandi</label>
            <div class="relative">
                <input type="password" id="password" name="password" required
                    placeholder="Min. 8 karakter (kombinasi huruf & angka)"
                    class="w-full bg-sand-50/80 border @error('password') border-rose-400 bg-rose-50/30 @else border-sand-200/90 @enderror rounded-2xl py-3 pl-11 pr-4 text-xs tracking-wide focus:outline-none focus:ring-2 focus:ring-emerald-900 focus:bg-white transition-all shadow-xs">
                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-sand-400">
        ...
        <div>
            <label for="password_confirmation" class="block text-[10px] font-bold uppercase tracking-[0.2em] text-sand-700 mb-1.5">Konfirmasi Kata Sandi</label>
            <div class="relative">
                <input type="password" id="password_confirmation" name="password_confirmation" required
                    placeholder="Ulangi kata sandi"
                    class="w-full bg-sand-50/80 border border-sand-200/90 rounded-2xl py-3 pl-11 pr-4 text-xs tracking-wide focus:outline-none focus:ring-2 focus:ring-emerald-900 focus:bg-white transition-all shadow-xs">
                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-sand-400">
        ...
        <div class="pt-2">
            <label class="flex items-start space-x-2.5 text-xs text-sand-600 cursor-pointer">
                <input type="checkbox" name="terms" value="1" required class="w-4 h-4 mt-0.5 rounded text-emerald-950 focus:ring-emerald-900 border-sand-300">
        ...
        <button type="submit" class="w-full py-4 px-6 bg-emerald-950 hover:bg-emerald-900 text-gold-300 font-bold rounded-2xl shadow-luxury border border-gold-500/30 hover:shadow-gold-glow transition-all text-xs uppercase tracking-widest flex items-center justify-center space-x-2 mt-2">
