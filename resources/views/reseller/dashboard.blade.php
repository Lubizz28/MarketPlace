...
<div class="space-y-8">
    ...
    <div class="bg-emerald-luxury rounded-3xl p-7 sm:p-10 text-white shadow-luxury-lg relative overflow-hidden border border-gold-500/30">
        ...
        <div class="relative z-10 space-y-3">
            <div class="flex flex-wrap items-center gap-2">
                <span class="text-[9px] uppercase tracking-[0.25em] font-bold px-3.5 py-1 bg-gold-gradient text-emerald-950 rounded-full shadow-xs">
                    ✦ Mitra Bisnis Reseller
                </span>
                @if($user->status->value === 'pending')
                    <span class="text-[9px] uppercase tracking-[0.2em] font-bold px-3 py-1 bg-amber-400 text-sand-950 rounded-full">
                        ⏳ Menunggu Verifikasi Admin
                    </span>
                @else
                    <span class="text-[9px] uppercase tracking-[0.2em] font-bold px-3 py-1 bg-emerald-400 text-emerald-950 rounded-full">
                        ✓ Terverifikasi Aktif
                    </span>
                @endif
            </div>
    ...
    <!-- Financial Cards with SVG Icons -->
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
        <div class="bg-white/95 backdrop-blur-xl p-6 rounded-3xl border border-gold-500/40 shadow-luxury ring-1 ring-gold-500/20">
            <div class="flex items-center justify-between">
                <span class="text-[9px] font-bold uppercase tracking-[0.2em] text-gold-700">Saldo Dompet Komisi</span>
            ...
            <p class="text-2xl font-display font-bold text-emerald-950 mt-2 font-mono">Rp 0</p>
        ...
        <div class="bg-white/95 backdrop-blur-xl p-6 rounded-3xl border border-sand-200/90 shadow-luxury">
            <div class="flex items-center justify-between">
                <span class="text-[9px] font-bold uppercase tracking-[0.2em] text-sand-400">Komisi Tertunda</span>
            ...
            <p class="text-2xl font-display font-bold text-sand-900 mt-2 font-mono">Rp 0</p>
            <span class="text-[11px] text-sand-400 font-light">Pesanan belum selesai</span>
        </div>

        <div class="bg-white/95 backdrop-blur-xl p-6 rounded-3xl border border-sand-200/90 shadow-luxury">
            <div class="flex items-center justify-between">
                <span class="text-[9px] font-bold uppercase tracking-[0.2em] text-sand-400">Transaksi Referral</span>
                <div class="w-8 h-8 rounded-xl bg-sand-100 flex items-center justify-center text-sand-700">
            ...
            <p class="text-2xl font-display font-bold text-sand-900 mt-2">0 Transaksi</p>
            <span class="text-[11px] text-sand-400 font-light">Dari tautan promosi Anda</span>
        </div>
    </div>
    ...
    <div class="bg-white/95 backdrop-blur-xl p-7 sm:p-8 rounded-3xl border border-sand-200/90 shadow-luxury space-y-4">
        <div>
            <span class="text-[9px] uppercase tracking-[0.25em] font-bold text-gold-700">Afiliasi &amp; Promosi</span>
            <h2 class="text-lg font-display font-bold text-sand-900 mt-1">Tautan Referral Eksklusif Anda</h2>
        </div>
        <p class="text-xs text-sand-600 leading-relaxed max-w-2xl font-light">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-2">
            ...
            <button type="button" onclick="navigator.clipboard.writeText('{{ url('/?ref=' . ($user->phone ?? 'reseller-' . $user->id)) }}'); alert('Tautan referral berhasil disalin!');"
                class="px-6 py-3.5 bg-emerald-950 hover:bg-emerald-900 text-gold-300 font-bold text-xs uppercase tracking-widest rounded-2xl shadow-luxury border border-gold-500/30 hover:shadow-gold-glow transition-all shrink-0 flex items-center justify-center space-x-2">
